...
begin
  Result := '';
  i := 1;
  While i<=Length(WorkStr) do // ”даление лишних пробелов
  begin
...
var
  ...
  var
    O1, O2 : Boolean;
  begin
    If Node^.lo = loNot then O1 := False else
      If Node^.i1 = nil then O1 := GetInput(Node^.i1Str) else
        O1 := Recurse(Node^.i1);

    If Node^.i2 = nil then O2 := GetInput(Node^.i2Str) else
      O2 := Recurse(Node^.i2);

    Case Node^.lo of
      loAnd : Result := O1 and O2; // Х
      loOr  : Result := O1 or  O2; // +
      else Result := not O2;       // ђ
    end;
  end;
begin
  Err := False;
  ...
  If Err then MessageBox(MainForm.Handle, 'ќшибка анализа формулы !',
    '“аблица истинности', mb_Ok or MB_ICONERROR);
end;
...
begin
  ...
  If Result <> '' then
  Case Node^.lo of
    loAnd : Result := Result + 'Х';
    loOr  : Result := Result + '+';
    else Result := Result + 'ђ';
  end;
  If Node^.i2 = nil then
